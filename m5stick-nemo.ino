...
void dmenu_setup() {
  DISP.fillScreen(BGCOLOR);
  DISP.setCursor(0, 5, 1);
  DISP.println("  DEFINIR TEMPO");
  DISP.println("");
  DISP.println("  ESCURECIMENTO");
  DISP.println("");
  DISP.println("   AUTOMATICO");
  delay(1000);
  cursor = 0;
  rstOverride = true;
...
MENU wsmenu[] = {
  { "Voltar", 5},
  { "Scan WI-FI / CLONE", 0},
  { "WI-FI Aleatorios ", 1},   //Spam Funny
  { "WI-FI 30", 2},            //Spam Rickroll
  { "WI-FI Cripto", 3},        //Spam Random
  { "WI-FI Portal", 4},         //NEMO Portal
};
...
void wscan_result_loop(){
  ...
  if (check_select_press()) {
    ...
    DISP.print("BSSID:\n" + WiFi.BSSIDstr(i));
    DISP.printf("\nBtn: Voltar\n");
    DISP.printf("Segure Para: Clonar\n");
   if(check_select_press()){
